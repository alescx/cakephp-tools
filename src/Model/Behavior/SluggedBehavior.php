...
class SluggedBehavior extends Behavior
{
    ...
    {
        $this->_defaultConfig['cacheConfig'] = [
            'className' => 'Redis',
            'server' => 'redis'
        ];
        ...
        if ($this->_defaultConfig['useCache'] && !\Cake\Cache\Cache::getConfig('_slugged_')) {
            \Cake\Cache\Cache::setConfig('_slugged_', $this->_defaultConfig['cacheConfig']);
        }
    ...
    public function initialize(array $config)
    {
        if ($this->_config['length'] === null) {
            $length = $this->_table->getSchema()->column($this->_config['field'])['length'];
     ...
     * @throws \Cake\Datasource\Exception\RecordNotFoundException if slug is not found
     */
    public function findSlugged(Query $query, array $options)
    {
        ...
        $slug = $options['slug'];
        $cachedSlugs = $this->getCachedSlugs();

        if (empty($cachedSlugs) || !array_key_exists($slug, $cachedSlugs)) {
            throw new RecordNotFoundException(__('Cannot find record'));
        }
        $id = $cachedSlugs[$slug];
        return $query->where([$this->_table->getAlias() . '.' . $this->_table->getPrimaryKey() => $id]);
    }

    public function getIdFromSlug($slug)
    {
        $cachedSlugs = $this->getCachedSlugs();
        return (isset($cachedSlugs[$slug]) ? $cachedSlugs[$slug] : null);
    }

    protected function getCachedSlugs()
    {
        $key = $this->_table->getTable();
        return (array)\Cake\Cache\Cache::remember($key, function () {
            return $this->_table->find('list', [
                'keyField' => $this->_config['field'],
                'valueField' => $this->_table->getPrimaryKey()
            ])->toArray();
        }, '_slugged_');
    }
    ...
    {
        if (!$this->_config['on'] === 'afterSave' || !$this->_defaultConfig['useCache']) {
            return;
        }

        $key = $this->_table->getTable();
        $slug = $entity->{$this->_config['field']};
        $id = $entity->{$this->_table->getPrimaryKey()};

        $slugs = $this->getCachedSlugs();
        $slugs[$slug] = $id;

        \Cake\Cache\Cache::write($key, $slugs, '_slugged_');
    }
